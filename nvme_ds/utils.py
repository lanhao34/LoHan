...
def swap_out_tensors(swap_handle, tensor_buffers, swap_paths):
    for buffer, path in zip(tensor_buffers, swap_paths):
        # print(buffer.device, buffer.size())
        # print('swap_out async pwrite')
...
class SwapBuffer(object):
    ...
    def insert_tensor(self, tensor, swap_path, aligned_numel):
        ...
        # print('compute_tensor.data', compute_tensor.data.device)

        @nvtx_wrap
        def test_just_copy():
            compute_tensor.data.copy_(tensor.data, non_blocking=False)
        test_just_copy()
...
class SwapBufferManager(object):
    ...
    def __init__(self, num_elems, count, dtype):
        ...
        self.count = count
        self.dtype = dtype
        see_memory_usage('BEF ini swap buffer !!!!!!!!!!!!!!')
        self.all_buffers = [
            torch.zeros(num_elems, device='cpu', dtype=dtype).pin_memory() for _ in range(count)
        ]
        see_memory_usage('AFT ini swap buffer !!!!!!!!!!!!!!')

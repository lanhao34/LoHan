...
from nvme_ds.utils import swap_in_tensors, swap_out_tensors, \
...
class OptimizerStateSwapInfo(object):
    ...
    def read_unswapped_gradients(self, dest_buffer):
        ...
        for offset, grad_partition in self.unswapped_gradients.items():
            ...
            dst_tensor.data.copy_(grad_partition.data, non_blocking=False)
...
class OptimizerSwapper(object):
    ...
    def __init__(self, swap_config, aio_config, base_folder, optimizer, largest_numel, device, dtype, timers):
        ...
        self.swap_folder = os.path.join(base_folder, 'optimizer')
    ...
    def _get_state_tensors(self, parameter):
        ...
        tensor_list = []
        # print('self.optimizer.state[parameter]', self.optimizer.state[parameter])
        # print('len(self.optimizer.state[parameter])', len(self.optimizer.state[parameter]))
        # for value in self.optimizer.state[parameter].keys():
            # print(value)
    ...
    def _update_param_state_info(self, swap_info, parameter):
        # print('before in _update_param_state_info')
        if not swap_info.has_state_tensors:
            # print(' in _update_param_state_info')

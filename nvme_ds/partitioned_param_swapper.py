...
class AsyncPartitionedParameterSwapper(object):
    ...
    def _track_numel(self, params):
        for param in params:
            assert param.manage is not None, "Partitioned tensor is None"
            self.param_id_to_numel[param.sb_id] = param.manage.ds_numel
    ...
    def synchronize_reads(self):
        ...
        for param, swap_in_buffer in zip(self.inflight_params, self.inflight_swap_in_buffers):
            ...
            param.manage.data = compute_buffer.data
            param.manage.nvme_status = PartitionedParamStatus.AVAILABLE

        self.available_params.update([param.sb_id for param in self.inflight_params])
    ...
    def remove_partition_and_release_buffers(self, params):
        for param in params:
            ...
            param.manage.data = self.invalid_buffer.data
            param.manage.nvme_status = PartitionedParamStatus.NOT_AVAILABLE
    ...
    def _update_inflight_swap_in(self, params, swap_in_buffers, inflight_numel):
        ...
        for param in params:
            param.manage.nvme_status = PartitionedParamStatus.INFLIGHT
    ...
    def swap_in(self, params, async_op=True, swap_in_buffers=None):
        ...
        assert all([param.manage.nvme_status == PartitionedParamStatus.NOT_AVAILABLE
    ...
    def swap_into_buffer(self, param, dest_buffer):
        assert param.manage.nvme_status == PartitionedParamStatus.NOT_AVAILABLE, f"param {param.sb_id} is already available or inflight"

        require_swap_buffer = not (dest_buffer.is_pinned() and self._is_io_aligned(dest_buffer.numel()))

        if require_swap_buffer:
            assert len(self.available_buffer_ids) > 0, f"No buffer available to swap param {param.sb_id}."
        ...
        if require_swap_buffer:
            dest_buffer.data.copy_(param.manage.data)
